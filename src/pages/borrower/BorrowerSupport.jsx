...
export default function BorrowerSupport() {
  ...
  useEffect(() => {
    if (!user) return;
    base44.entities.SupportRequest.filter({ borrowerEmail: user.email }, '-created_date').then(setRequests);
  }, [user]);

  const handleSubmit = async () => {
    if (!type || !message) return toast.error('Please fill in all fields');
    setSubmitting(true);
    const created = await base44.entities.SupportRequest.create({
            ...
            <CardContent className="py-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">{new Date(s.created_date).toLocaleDateString()}</p>
                </div>
