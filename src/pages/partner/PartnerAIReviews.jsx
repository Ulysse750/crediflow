...
export default function PartnerAIReviews() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="AI Document Reviews" description="AI-assisted analysis of borrower documents" />
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  {a.humanReviewRequired && (
                    <span className="text-xs text-amber-600 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Human review required</span>
                  )}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div><p className="text-xs text-muted-foreground font-medium">Missing Documents</p><p>{a.missingDocuments.length > 0 ? a.missingDocuments.join(', ') : 'None'}</p></div>
                </div>
                {a.inconsistencies.length > 0 && (
                  <div><p className="text-xs text-muted-foreground font-medium">Inconsistencies</p>
                    <ul className="list-disc list-inside text-amber-700">{a.inconsistencies.map((c, i) => <li key={i}>{c}</li>)}</ul>
                  </div>
                )}
                {a.followUpQuestions.length > 0 && (
                  <div><p className="text-xs text-muted-foreground font-medium">Follow-up Questions</p>
